...
pos = 'abcdef'
fig, axes = plt.subplot_mosaic('abc;def', figsize=(18.5/2.54, 18.5/3*2/2.54), constrained_layout=True)
...
for i, chip in enumerate(chips):
    ...
    ax.annotate('$\epsilon_f$', xy=(8, 50), ha='center', va='center', color='k', fontsize='small')
    ax.annotate('$\epsilon_M-\epsilon_f$', xy=(7.75, 20), ha='center', va='center', color=cs2[i], fontsize='small')
    ...
    ax.hist(chip.df_f_fit*1e3, facecolor='o', alpha=.75, label='data', orientation='vertical', bins=bins)
    # chip.remove_spatial()
    ...
    ax.annotate('$\epsilon_M^*=\epsilon_M-\epsilon_f$', xy=(0.6, 0.6+(1-i)*0.2), xycoords='axes fraction', ha='center', va='center', color='o', fontsize='small')
    ax.annotate('$\sigma_M^*=%.2G\\times 10^{-3}$' % (np.nanstd(chip.df_f_fit)*1e3), xy=(0.6, 0.5+(1-i)*0.2), xycoords='axes fraction', ha='center', va='center', color='o', fontsize='small')
...
cbar = fig.colorbar(axes['b'].imshow(empty_array, origin='lower', cmap=cmap, clim=clim), ax=axes['b'], orientation='horizontal', location='bottom', pad=0)
...
axes['c'].hist(chip.df_f*1e3, facecolor=c, bins=bins, alpha=1, zorder=0)
ylim = 0, 250
clim = -2.5, .5
cticks = [-2, -1, 0]
binsize = 0.075
