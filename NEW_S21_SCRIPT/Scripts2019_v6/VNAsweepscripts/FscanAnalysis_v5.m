function [arrayparam,KIDparam] = FscanAnalysis_v4(inputmat,fileparam)
% NewestVersion JB 09-03-2017
% allows for fitting (optional)
...
LowQc = 100;
HighQc = 1e6;  %invalid Qc params
% fitting dips
fitit=0;    %0 no fit, 1 fit
no=1;       %BW to fit pver
plotfit=0;  %plot fitresult (debug only, default 0
%Cross talk analysis (might crash od nonzero)
DoCT=0;
%detailed cross talk analysis (do only for good arrays where dF > sigma(df)
dodetailedFanalysis = 1;
sigmafact = 2.7;%3.5 - 3. 3 iterations to remove outliers outside this range in sigma
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% give file and dir for firect data import, ignored if fy is called with arguments
filename='400IRAM.dat';
...
histmax=10;         %DEFAULT=20 maximum df value in the histogram (in MHz)

%variables to find the dips (might need some playing with)
diff2treshold=0;  %This one needs manual adjustment. Set to 0 for automatic
%fort poltting
plotdots=1;

%variables that are normaly ok (for dip finding)
difsmooth=30;       %Typ. 10. difsmooth is a smoothing in the derivative fundtion used to find the KID peaks, typ=10-100%
...
%find diff2treshold if not existing
if diff2treshold == 0
    diff2treshold = 10*std(der2(1:100,2));    %10 sigma treshold
...
for c=1:numpeaks
    if foundpeak
        ...
        if c>1
            ...
            KIDparam.df_f(c)=abs((KIDparam.fres(c)-KIDparam.fres(c-1))/(0.5*KIDparam.fres(c)+0.5*KIDparam.fres(c-1)));%using Liu et al., arXiv:1711.07914v1
        else
            KIDparam.df(c)=0;
            KIDparam.df_f(c) = 0;
        end
        ...
        if fitit==1
            fdata=[freq(peaks_10BW{c})*1e9 20*log10(magn_syscal(peaks_10BW{c}))];
            [result,fitted] = FitS21simple(fdata,KIDparam.fres(c)*1e9,KIDparam.Q(c),KIDparam.S21min(c),no);
            if plotfit==1;
                figure(100);
                clf
                plot(result);hold on;plot(fdata(:,1),fdata(:,2),'.');hold off;
            end
        end
    else
...
%calculates means
[bla,blaind]=find(KIDparam.Qc<LowQc | KIDparam.Qc>HighQc);%invalid Qc
crazykids=length(bla);
if crazykids>0
    disp(['Crazy Qc found: number= ' num2str(length(bla))])
end
[bla,blaind]=find(KIDparam.Qc>LowQc & KIDparam.Qc<HighQc);%decent ones with decent Qc
...
[~,blaind2]=find(KIDparam.Qc(2:end)>LowQc & KIDparam.Qc(2:end)<HighQc);%invalid Qc 2:end for dF
arrayparam.meandf=mean(KIDparam.df(blaind2));
arrayparam.stddf=std(KIDparam.df(blaind2));
arrayparam.meandf_f=mean(KIDparam.df_f(blaind2));
arrayparam.stddf_f=std(KIDparam.df_f(blaind2));
arrayparam.df10MHz=length(find(KIDparam.df(blaind2)<0.01))-1;%number dF below 5 MHz. -1 beacuse df first KID with itself=0.
arrayparam.df5MHz=length(find(KIDparam.df(blaind2)<0.005))-1;%number dF below 5 MHz. -1 beacuse df first KID with itself=0.
arrayparam.df1MHz=length(find(KIDparam.df(blaind2)<0.001))-1;%number dF below 1 MHz. -1 beacuse df first KID with itself=0.
arrayparam.df02MHz=length(find(KIDparam.df(blaind2)<0.0002))-1;%number dF below 0.2 MHz. -1 beacuse df first KID with itself=0.
...
fprintf('# Q analysis performed on = %.0f  MKIDs \n', numpeaks-crazykids);
...
figure(4)
subplot(2,2,1)
semilogy(KIDparam.Qi(blaind),'bx');
hold on;
semilogy(KIDparam.Q(blaind),'ro');
semilogy(KIDparam.Qc(blaind),'k.');
xlabel('index')
ylabel('Q factors')
legend('Qi','Q','Qc')
subplot(2,2,2)
hist(KIDparam.df(blaind)*1000,0:histdf:histmax)
xlim([0 histmax])
xlabel('df [MHz]')
ylabel('count')
title(['mean ' num2str(arrayparam.meandf*1000)  'std '  num2str(arrayparam.stddf*1000)])
subplot(2,2,3)
hist(KIDparam.Qc(blaind),min(KIDparam.Qc(blaind)):max(KIDparam.Qc(blaind)-min(KIDparam.Qc(blaind)))/30 :max(KIDparam.Qc(blaind)))
xlabel('Qc ')
ylabel('count')
title(['mean ' num2str(arrayparam.meanQc)  'std '  num2str(arrayparam.stdQc)])
subplot(2,2,4)
hist(20*log10(KIDparam.S21min(blaind)),-40:2:0)
xlim([-40 0])
xlabel('S21min [dB]')
ylabel('count')
title(['mean ' num2str(arrayparam.meanS21dB)  'std '  num2str(arrayparam.stdS21dB)])

%detailed df abalysis (only do for good arrays)
if dodetailedFanalysis == 1
    figure(5)
    subplot(1,2,1)
    histrange = max(KIDparam.df_f(blaind));
    h1=histogram(KIDparam.df_f(blaind),40);
    hold on;
    xlim([0 histrange])
    xlabel('\delta f/f')
    ylabel('count')
    %
    meanv = arrayparam.meandf_f;
    stdv = arrayparam.stddf_f;
    df_f = KIDparam.df_f(blaind);
    for n=1:3
        %sigmafact = 2.7;
        df_f = df_f(df_f > meanv - sigmafact*stdv & df_f < meanv + sigmafact*stdv);
        meanv = mean(df_f);
        stdv = std(df_f);
    end
    h2 = histogram(df_f,h1.BinEdges);
    Xvalsdf_f = h2.BinWidth*(0.5+0:1:h2.NumBins);
    Yvalsdf_f = h2.Values
    %plot(Xvalsdf_f,Yvalsdf_f,'kx');
    %Fit Gauss curve
    f = fit(Xvalsdf_f',Yvalsdf_f','gauss1');
    xx = 0:(meanv+5*stdv)/100 :meanv+5*stdv;
    plot(xx,feval(f,xx),'r','Linewidth',4)
    title(['from fit: \mu ' num2str(f.b1)  ', \sigma = '  num2str(f.c1)]);
    arrayparam.meandf_f = f.b1;
    arrayparam.stddf_f  = f.c1;
    
    subplot(1,2,2)
    histrange = max(KIDparam.df(blaind));
    h1=histogram(1e3*KIDparam.df(blaind),40);%in MHz
    hold on;
    xlim(1e3*[0 histrange])
    xlabel('\delta f  [MHz]')
    ylabel('count')
    %
    meanv = 1e3*arrayparam.meandf;%in MHz
    stdv = 1e3*arrayparam.stddf;%in MHz
    df = 1e3*KIDparam.df(blaind);%in MHz
    for n=1:3
        %sigmafact = 2.7;
        df = df(df > meanv - sigmafact*stdv & df < meanv + sigmafact*stdv);
        meanv = mean(df);
        stdv = std(df);
    end
    h2 = histogram(df,h1.BinEdges);%in MHz
    Xvalsdf = h2.BinWidth*(0.5+0:1:h2.NumBins);%in MHz
    Yvalsdf = h2.Values;
    plot(Xvalsdf,Yvalsdf,'kx');
    %Fit Gauss curve
    f = fit(Xvalsdf',Yvalsdf','gauss1');%in MHz
    xx = 0:(meanv+5*stdv)/100 :meanv+5*stdv;%in MHz
    plot(xx,feval(f,xx),'r','Linewidth',4)
    title(['from fit: \mu ' num2str(f.b1)  ' MHz, \sigma = '  num2str(f.c1) ' MHz']);
    arrayparam.meandf = f.b1;%in MHz
    arrayparam.stddf  = f.c1;%in MHz
    set(gcf,'Color','White')
    saveas(figure(5),[dir fileparam 'Figure5_detailed'  '.fig']);
...
%% cross talk analysis, slow
if DoCT==1
    close all
    tba = find(KIDparam.S21min<0.98);
    KIDparam.ct = zeros(size(KIDparam.Qc));
    CT1 = zeros(size(KIDparam.Qc));
    CT2 = zeros(size(KIDparam.Qc));
    for n=1:length(tba)-1
        %get the cross talk of MKID n+1 on n => how much do we see n+1 reading out n => n+1 is cross talking
        CT1(tba(n+1)) = simulate2KIDsV2_copyJB(KIDparam.Qc([tba(n) tba(n+1)]),KIDparam.Qi([tba(n) tba(n+1)]),0,KIDparam.df(2)/KIDparam.fres(1));
    end
    for n=1+1:length(tba)
        %get the cross talk of MKID n-1 on n => how mucgh do we see n-1 reading out n => n-1 is cross talking
        CT2(tba(n-1)) = simulate2KIDsV2_copyJB(KIDparam.Qc([tba(n) tba(n-1)]),KIDparam.Qi([tba(n) tba(n-1)]),0,KIDparam.df(2)/KIDparam.fres(1));
    end
    KIDparam.ct=max([CT1 CT2]);%we add the cross talk levels from MKID to the left and MKID to the right
    arrayparam.meanct = mean(KIDparam.ct(~isnan(KIDparam.ct)));
    
    figure(5)
    set(gcf,'Position', [10, 10, 1200, 800]);
    subplot(1,3,1)
    semilogy((KIDparam.ct),'.')
    grid on
    xlabel('KID index')
    ylabel('cross talk with both neighbours')
    subplot(1,3,2)
    histogram(100*KIDparam.ct,'BinLimits',100*[0,1.5e-3]);
    xlabel('cross talk percentage')
    title(['<ct> = ' num2str(round(100*arrayparam.meanct*1000)/1000)  ' %, based on ' num2str(length(tba)) ' resonators'])
    subplot(1,3,3)
    semilogy(KIDparam.fres,KIDparam.ct,'.')
    xlabel('Fres  (GHz)')
    ylabel('cross talk with both neighbours')
    title(['#MKIDs with ct > 1e-3 = ' num2str((sum([KIDparam.ct]>1e-3)))])
    axis tight
    grid on
    %savin CT
    saveas(figure(5),[dir fileparam 'Figure5_crosstalk'  '.fig']);
    
    fprintf(fid,'mean cross talk level: %.4f \n', arrayparam.meanct);
    fclose(fid);
...
function ct = simulate2KIDsV2_copyJB(Qc,Qi,mkplts,dF_Fo)

% simulate2KIDs(Qc,Qi)
% This script calculates the Lorentzian of 2 MKIDs in complex space and shows the cross coupling levels.
% calculate sthe cross talk of MKID 2 on MKID 1
% It uses the MKID with index c=1 to center in the complex plane and
% calcualtes the cross talk from KID 2 on MKID 1.
% outputs figures and ct struct
% V2: 2 different Q factors possible
% V2: plotting can be disabled
% Qc = coupling Q [Q1 Q2]
% Qi = internal Q [Q1 Q2]
% mkplts = 1 to make plots (stand alone) 0 no plots
% dF_Fo is normalized resonance frequency difference between MKIDs you want
% to know the ct level of
%
% ct = cross talk estimate for 2 KIDs with given parameters
%%%%%%%%%%%%%%%%%%
lll={'-b';'-r';'-k'};ll={'xb';'xr';'xk'};
if length(Qc) ~=2 || length(Qi)~=2
    error('Q and Qi are 1x2 arrays')
end
F0=6.00e9;%Hz%lowest freq of the2 MKIDs, not relevant, all outcomes are normalized
dFfrac=1; %between 2 resonators in fractions of BW for the plot
%%%%%%%%%%%%%%%%%
% KID params
Q=Qc.*Qi./(Qc+Qi);
BW=F0./Q;
Smin=Q./Qi;
dF=dFfrac*max(BW);
fres(1)=F0;fres(2)=F0+dF;
% Franges for figures
minf=fres(1)-20*max(BW);maxf=fres(2)+20*max(BW);
df=0.01*min(BW);
ff=minf:df:maxf;
%% preparing MKID resonances to plot
S21=zeros(length(ff),2);S21c=S21;S21c_2=zeros(length(ff),1)+1;
for c=1:2
    dx=(ff-fres(c))/fres(c);%-2BW..2BW+dF  -2BW-dF...2BW
    S21(:,c)=10*log10( 1-(1-Smin(c)^2)./(1+(2*Q(c)*dx).^2) );%logMag
    S21c(:,c)=(Smin(c)+2*1i*Q(c)*dx)./(1+2*1i*Q(c)*dx);%complex
    S21c_2=S21c_2.*S21c(:,c);%addition of complex S21 curves of 2 MKIDs
end
%% going to complex plane MKID centered. We use MKID 1
S21c(:,:)=S21c(:,:)-(1+Smin(1))/2;
Smin2=min(S21c_2);
%S21c_2=S21c_2(:,1)-(1+Smin2)/2;

%% now start playing with derivatives
%Spline the MKIDs
% for c=1:2
%     S21sp{c} = spline(ff,[0 S21c(:,c)' 0]);% spline with 0 der at endsand fnder
%     S21c_splined(:,c)=ppval(S21sp{c},ff);
% end

for c=1:2
    S21sp{c} = spline(ff,[0 S21c(:,c)' 0]);% spline with 0 derivative at ends of curve of complex response
    S21c_splined(:,c)=ppval(S21sp{c},ff);%splined addition of complex S21 curves of each MKID
    %Angle response
    Phasesp{c} = spline(ff,[0 unwrap(angle(S21c(:,c)'))/pi/2 0]);% % spline with 0 derivative at ends of curve
    ddPhasesp{c} = fnder(Phasesp{c},1);%derivative of spline of KID phase of each MKID
    ddPhasesp_splined(:,c)=ppval(ddPhasesp{c},ff);%splined derivative of spline of KID phase of each MKID
    
end

%combinesResp=(ddPhasesp_splined(:,1)+ddPhasesp_splined(:,2))/max(ddPhasesp_splined(:,1));
CrossTalkResp=ddPhasesp_splined(:,2)./ddPhasesp_splined(:,1); %resp from KID 2 norm to response of KID1; cross talk of 2 on 1
for c=1:2
    [~,F0i(c)]=min(abs(fres(c)-ff));%F0i is the index of resonance frequencies
end
%% %%%%% FIGURE %%%%%
if mkplts == 1
    figure(2)
    subplot(2,2,1)
    % plotting individual KIDs
    for c=1:2
        plot((ff-F0),S21(:,c),lll{c},'LineWidth',2);hold on
    end
    plot((ff-F0),20*log10(abs(S21c_2)),'-k','LineWidth',2);hold on
    axis tight
    ylabel('S21 (dB)');xlabel('F/F0')
    legend('MKID 1','MKID 2','Combined S21')
    
    subplot(2,2,2)
    plot(real(S21c_2),imag(S21c_2),'k','LineWidth',2);hold on
    
    plot(real(S21c_2(F0i(1))),imag(S21c_2(F0i(1))),'bo','MarkerFaceColor','b');
    plot(real(S21c_2(F0i(2))),imag(S21c_2(F0i(2))),'ro','MarkerFaceColor','r');
    xlabel('Re');ylabel('Im')
    legend('Combined S21','Fres1','Fres2')
    subplot(2,2,3)
    for c=1:2
        plot(ff,unwrap(angle(S21c_splined(:,c)))/pi/2,lll{c});hold on
    end
    xlabel('F (Hz)');ylabel('\theta  (2\pi rad)')
    
    subplot(2,2,4)
    for c=1:2
        semilogy(ff,ddPhasesp_splined(:,c)/max(ddPhasesp_splined(:,1)),lll{c},'LineWidth',2);hold on
        
    end
    
    
    semilogy(ff,CrossTalkResp,'k','LineWidth',2);hold on
    ylim([0.001 1.2]);grid on;
    xlabel('F (Hz)');ylabel('d\theta/dF  (2\pi rad)')
    legend('MKID 1','MKID 2','CrossTalk of 2 on 1')
    title('best cross talk when KID optimal biassed')
    
end
%%%%%% end FIGURE %%%%%
%% Get cfross talk vs dF of MKID 2 on MKID 1: we read out 1 and calcualte the response of dF(2) on phase(MKID1)

ffrange= ff(F0i(2):end-5);%F0i(2) is the index of resonance frequencie of MKID 2
onepixel=ddPhasesp_splined(:,2)/max(ddPhasesp_splined(:,1));% phase responsivity MKID2 normalized to max response MKID 1
onepixct=onepixel(F0i(2):end-5)';
a=find(onepixct<0.1,1,'first');
aa=find(onepixct<10^-1.5,1,'first');
aaa=find(onepixct<0.01,1,'first');
aaaa=find(onepixct<0.001,1,'first');

%% %%%%% FIGURE %%%%%
if mkplts == 1
    figure(1)
    semilogy((ffrange/fres(2)-1),onepixct,lll{1},'LineWidth',2);hold on;
    xlabel('dFres/F0(1)-1');ylabel('Cross talk level');
    semilogy((ffrange(a)/fres(2))-1,onepixct(a),'bo','MarkerFaceColor','b');
    semilogy((ffrange(aa)/fres(2))-1,onepixct(aa),'ro','MarkerFaceColor','r');
    semilogy((ffrange(aaa)/fres(2))-1,onepixct(aaa),'go','MarkerFaceColor','g');
    semilogy((ffrange(aaaa)/fres(2))-1,onepixct(aaaa),'ko','MarkerFaceColor','k');
    axis tight;grid on
    legend('Cross talk',['-10dB @' num2str((ffrange(a)/F0)-1) ' dx-1'],['-15dB @' num2str((ffrange(aa)/F0)-1) ' dx-1'],...
        ['-20dB @' num2str((ffrange(aaa)/F0)-1) ' dx-1'],['-30dB @' num2str((ffrange(aaaa)/F0)-1) ' dx-1'])
    title(['cross talk on resonator 1 with Q= ' num2str(round(Q(1))) ...
        'due to MKID 2 with Q= ' num2str(round(Q(2)))])
end
%% output
[~,indct] = min(abs(ffrange/fres(2) - dF_Fo - 1));
ct = onepixct(indct);
