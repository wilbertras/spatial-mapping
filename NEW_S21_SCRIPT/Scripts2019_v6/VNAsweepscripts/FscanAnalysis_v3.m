function [arrayparam,KIDparam] = FscanAnalysis_v3(inputmat,fileparam)
...
filename='VNAsweep.dat';
dir='/Volumes/SpaceKids/FP7 SpaceKids/ExperimentsMOSAIC/LT109SParse/';%directory incl \ at end

%variables for the hystograms
histdf=0.2;         %DEFAULT=0.5. df for Histgram plot in MHz. 0.1 for high Q>100.000
histmax=10;         %DEFAULT=20 maximum df value in the histogram (in MHz)

%variables to find the dips (might need some playing with)
diff2treshold=0.50e8;  %80for hot ones use 0.4e8 100 mK use 1e8;  %Typ. 0.5-5e8. diff2treshold is treshold value in second derivation to find KIDs. 5e8 or so %
