%%
function [fm,SRRraw,SPPraw,SPRraw,nrrejected,sigmax5] = binfilefunction2(filename,rows,freq)
...
% rows: number of rows of the binfile (if cal is used they have to be equal size)
...
while telgood < 2  %this is to catch that all data is filtered, it iteratively increases TDoption.nrsigma until some data gets through  
    ...
    sigmaarray = zeros(1,TDoption.average);
    for k = 1:TDoption.average
        sigmaarray(k) = std(Rmmsmooth(1,(k-1)*n/TDoption.average+1:k*n/TDoption.average));
    end
    stdRmmsmooth = min(sigmaarray)*1.2;
    stdPmmsmooth = min(sigmaarray)*1.2;
    sigmax5 = stdRmmsmooth*TDoption.nrsigma;%for output
    ...
    for k = 1:TDoption.average
        ...
        if (sum(stdcheckR) > 0) || (sum(stdcheckcalR) > 0) || (sum(stdchecksmoothR) > 0) || (sum(stdcheckcalsmoothR) > 0) 
            %REJECT
            %set values in rejected range to 0 just for plotting, to show
            %which parts are rejected.
            index = find(stdcheckR);
            R_reject(1,((k-1)*n/TDoption.average+1)+index) = Rmm(1,((k-1)*n/TDoption.average+1)+index);
            Rmm(1,((k-1)*n/TDoption.average+1)+index) = 0;
            P_reject(1,((k-1)*n/TDoption.average+1)+index) = Pmm(1,((k-1)*n/TDoption.average+1)+index);
            Pmm(1,((k-1)*n/TDoption.average+1)+index) = 0;
            Rmmsmooth(1,((k-1)*n/TDoption.average+1)+index) = 0;
            Pmmsmooth(1,((k-1)*n/TDoption.average+1)+index) = 0;
        else
...
subplot(1,2,1)
plot(t,Rmmsmoothorig,tplot,minstdplus,'-g',tplot,minstdmin,'-g');
hold
plot(t_R_over,R_reject_smooth_over,'xk')
title('R_smooth(t) - mean(R) filtered with quasiparticle lifetime')
legend('smoothed time domain data',['R filtered +' num2str(TDoption.nrsigma) '\sigma'],['R smooth filtered -' num2str(TDoption.nrsigma) '\sigma'],'Rejected points')

subplot(1,2,2)
plot(t,Pmmsmoothorig,tplot,minstdplus,'-g',tplot,minstdmin,'-g');
hold
plot(t_P_over,P_reject_smooth_over,'xk')
title('P_smooth(t) - mean(P) filtered with quasiparticle lifetime')
legend('smoothed time domain data',['R filtered +' num2str(TDoption.nrsigma) '\sigma'],['P smooth filtered -' num2str(TDoption.nrsigma) '\sigma'],'Rejected points')

Figfiletd = [filename(1:end-4) '-plot_peak_rejection.fig'];%use the filename from the bin-file
