...
S21subdir='/Test_S21_2D/';   
%Note: pwd is current matlab directory. Usually directory where this m-file
%is located.
if nargin==0
    ChipInfo.path = ...                                         %Root path containing the measurement data.
    [cd '/../']
    %'\\MARS\kid\KIDonSun\experiments\Entropy ADR\LT186-Chip14'
    %'\\MARS\kid\KIDonSun\AAA_Scratch temp copies\tempPieter\LT179_chip4_S21'
    %'\\MARS\kid\KIDonSun\experiments\Entropy ADR\LT132_SiO2patch_30KIDs'
    %'\\MARS\kid\KIDonSun\experiments\Entropy ADR\LT165_W2_Chip10'
    %'\\MARS\kid\KIDonSun\experiments\Entropy ADR\LT165_Chip7VISmembraneIDC'  
    

    %'\\MARS\kid\KIDonSun\experiments\Entropy ADR\LT139_VIS-aperture'; %LT139
    ...
    ChipInfo.ResonatorInfoFile = ...                            %Name of the file containing resonator information
        [ChipInfo.path 'Design_empty.txt'];
    ChipInfo.FilmTc = 0.41;                                    %Transition temperature measured for the film [K].
	ChipInfo.material = 'Ta';                          %Material of which the resonator active area is made.
    ChipInfo.Thickness = 0.04;                                  %Thickness of the resonator active area [um].
    ChipInfo.Linewidth = 2; %linewidth in um, assumes that 4th colum of txt is width, not area
end
...
stopoffset=42;    %Removes the highest temperatures from the data analysis.
...
TCfrac=1/8;      %Fraction of Tc from where the responsivity fit starts. Default: 1/8
...
% 2 == use the function by Khalil et.al. to fit to |S21|(dB) 
%       this fit includes a slope correction, which does not make sense
% 3 == use the function by Khalil et.al. to fit to |S21| 
%       this fit is on linear scale and only corrects for a multiplicative factor (because labview normalises S21 to 1 at is max, which is not the case in the presence of throughline fase shift)    
...
%     if any(any(imag(S21data{p})))
%         S21data{p} = real(S21data{p});
%         disp('Warning: S21 data column small imaginary part negelected')
%     end
end
...
elseif strcmp(ChipInfo.material, 'TiN-highres')
    N0=8.7*10^9;%for TiN, Leduc APL 2010 in mu-3 eV-1
    delta=ChipInfo.FilmTc*k*1.76;%Need to update if we know this better
...
drdb=2*(1+Qc(1)/Qi(1))*Qc/2./(1+Qc./Qi).^2; %DIT KLOPT NIET, het moet 2*Q(T) zijn.
...
MakeGoodFigure(15,17,13,Figfile,0);
